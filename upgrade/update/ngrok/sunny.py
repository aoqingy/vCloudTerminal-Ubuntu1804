...
def ReqTunnel(ReqId, Info):
    Payload = dict()
    Payload['ReqId'] = ReqId
    Payload['Protocol'] = Info['protocol']
    Payload['Hostname'] = Info['hostname']
    Payload['Subdomain'] = Info['subdomain']
    Payload['HttpAuth'] = Info['httpauth']
    Payload['RemotePort'] = Info['rport']
    body = dict()
    body['Type'] = 'ReqTunnel'
    body['Payload'] = Payload
    buffer = json.dumps(body)
                    ...
                    if type == 1:
                        ...
                        if js['Type'] == 'AuthResp':
                            ...
                            for info in Tunnels:
                                # 注册通道
                                reqid = getRandChar(8)
                                sendpack(sock, ReqTunnel(reqid, info))
